...
_DEFAULT_NIGHT_WATCH_CODES = ["016665", "018147", "012922", "017437", "017731", "024239", "014002", "021277", "000041", "019075"]
...
def _extract_night_watch_codes(portfolio: dict) -> list[str]:
    ...
    if not isinstance(raw, list):
        return list(_DEFAULT_NIGHT_WATCH_CODES)
    seen: set[str] = set()
    result: list[str] = []
    for item in raw:
        ...
        seen.add(code)
        result.append(code)
    return result or list(_DEFAULT_NIGHT_WATCH_CODES)


async def _lookup_fund_meta(code: str) -> dict:
    """按 code 补充基金基础信息，避免夜盘列表仅有编号没有名称。"""
    try:
        results = await _get("/api/search", params={"key": code})
    except Exception:
        return {}
...
async def get_night_watchlist() -> dict:
    ...
    for code in codes:
        ...
        if not fund:
            fund = await _lookup_fund_meta(code)
        items.append({
            "code": code,
            "name": fund.get("name", ""),
            "type": fund.get("type", ""),
